-- 065: la limpieza fallaba con «DELETE requires a WHERE clause».
--
-- ⚠️⚠️ LA LECCIÓN, QUE VALE PARA TODA PRUEBA FUTURA CONTRA EL REMOTO:
-- la 064 se ensayó en transacción revertida y salió bien, pero **se ensayó con
-- el usuario equivocado**. El MCP conecta como `postgres`; la API entra por
-- PostgREST, cuyo rol `authenticator` lleva:
--
--   session_preload_libraries = supautils, safeupdate
--   statement_timeout = 8s
--
-- `safeupdate` rechaza cualquier DELETE o UPDATE **sin cláusula WHERE**, y la
-- 064 hacía `delete from pedidos;` a secas. Como `postgres` no precarga esa
-- librería, el ensayo pasó y el botón falló con 400 en cuanto se pulsó.
--
-- Probar en transacción revertida NO SIRVE DE NADA si se prueba con un usuario
-- que no tiene las mismas protecciones que el que va a ejecutarlo de verdad.
-- Las dos veces que salió en los logs (20:58:08 y 20:58:15) son el mismo error.
--
-- El arreglo es dar a cada DELETE un predicado real. No vale «pues le pongo
-- `where true`»: se escribe sobre una columna NOT NULL de cada tabla, que es un
-- predicato de verdad y no un rodeo para engañar a la protección.
--
-- ⚠️ Y el `statement_timeout = 8s` es el otro dato a recordar: hoy hay 3
-- pedidos y esto es instantáneo, pero una limpieza sobre miles de filas se
-- cortaría a la mitad. Si algún día pasa, hay que trocearla — no subir el
-- timeout, que es lo que protege a la API de quedarse colgada.
...
begin
  ...
  -- El WHERE de cada uno es lo que arregla el 400. `checkout_datos` se
  -- identifica por `session_id`, que es su clave; las demás por `id`.
  ...
  movidos as (
    -- Este UPDATE ya tenía WHERE, así que safeupdate nunca se quejó de él.
    update productos pr
