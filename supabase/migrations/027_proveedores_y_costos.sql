-- 027: Proveedores + costos en las compras de mercancía.
-- Submódulo de proveedores (CRUD dentro del módulo 'compras'): al registrar
-- una compra basta el CIF para traer los datos del proveedor. Las líneas de
-- compra llevan costo unitario y la factura guarda el total, calculado en la
-- transacción (fuente de verdad en BD).
...
-- ============================================================
-- 2. Compras: vínculo al proveedor + costos
--    (la columna `proveedor` existente queda como snapshot del nombre)
-- ============================================================
...
-- ============================================================
-- 3. RPC v2: compra + líneas (con costo) + stock + total, todo o nada
-- ============================================================

drop function if exists public.registrar_factura_compra(text, jsonb, text, text, text, uuid);

create function public.registrar_factura_compra(
  p_pdf_path       text,
  p_items          jsonb,
