...
export function TransferenciaCheckout({ total, payload, diasPlazo, disabled }: Props) {
  ...
  /*
   * La clave que hace idempotente el alta del pedido. Se genera una sola vez
   * por montaje del componente: aquí no hay una pasarela que devuelva un
   * identificador con el que reconocer el intento, así que si el cliente pulsa
   * dos veces —o la respuesta se pierde y reintenta— sin esto se crearían dos
   * pedidos con el mismo carrito y las dos reservas de stock.
   */
